...
from vecnadb.modules.pipelines import Task, run_pipeline
from vecnadb.modules.pipelines.layers.resolve_authorized_user_dataset import (
    resolve_authorized_user_dataset,
)
from vecnadb.modules.pipelines.layers.reset_dataset_pipeline_run_status import (
    reset_dataset_pipeline_run_status,
)
from vecnadb.modules.engine.operations.setup import setup
from vecnadb.tasks.ingestion import ingest_data, resolve_data_directories
...
async def add(
    ...
    Next Steps:
        ...
        await vecnadb.add("Your document content")  # Raw text
        # OR
        await vecnadb.add("/path/to/your/file.pdf")  # File path

        # Step 2: Process into knowledge graph
        await vecnadb.cognify()

        # Step 3: Search and query
        results = await vecnadb.search("What insights can you find?")
        ```

    Example Usage:
        ```python
        # Add a single text document
        await vecnadb.add("Natural language processing is a field of AI...")
        ...
        # Add a single file
        await vecnadb.add("/home/user/documents/analysis.pdf")
        ...
        await vecnadb.add("https://example.com",extraction_rules=extraction_rules)
        ...
        await vecnadb.add("https://example.com")

        # Add multiple urls
        await vecnadb.add(["https://example.com","https://books.toscrape.com"])
        ```
