...
def download_frontend_assets(force: bool = False) -> bool:
    ...
    try:
        ...
        if "404" in str(e):
            ...
            logger.error("Try using a stable release version of vecnadb.")
        else:
...
def find_frontend_path() -> Optional[Path]:
    ...
    dev_search_paths = [
        current_file.parents[4] / "cognee-frontend",  # from vecnadb/api/v1/ui/ui.py to project root
...
def start_ui(
    ...
    Example:
        ...
        >>> server = vecnadb.start_ui(dummy_callback)
        >>>
        >>> # Start both frontend and backend
        >>> server = vecnadb.start_ui(dummy_callback, start_backend=True)
        >>> # UI will be available at http://localhost:3000
        >>> # API will be available at http://localhost:8000
        >>>
        >>> # Start frontend with MCP server
        >>> server = vecnadb.start_ui(dummy_callback, start_mcp=True)
        >>> # UI will be available at http://localhost:3000
    ...
    if not frontend_path:
        logger.info("Frontend not found locally. This is normal for pip-installed vecnadb.")
