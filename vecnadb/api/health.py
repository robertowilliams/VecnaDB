...
from vecnadb.version import get_cognee_version
...
class HealthChecker:
    ...
    async def check_relational_db(self) -> ComponentHealth:
        ...
        try:
            from vecnadb.infrastructure.databases.relational.get_relational_engine import (
                get_relational_engine,
            )
            from vecnadb.infrastructure.databases.relational.config import get_relational_config
    ...
    async def check_vector_db(self) -> ComponentHealth:
        ...
        try:
            ...
            from vecnadb.infrastructure.databases.vector.config import get_vectordb_config
    ...
    async def check_graph_db(self) -> ComponentHealth:
        ...
        start_time = time.time()
        try:
            from vecnadb.infrastructure.databases.graph.get_graph_engine import get_graph_engine
    ...
    async def check_file_storage(self) -> ComponentHealth:
        ...
        try:
            import os
            from vecnadb.infrastructure.files.storage.get_file_storage import get_file_storage
    ...
    async def check_llm_provider(self) -> ComponentHealth:
        ...
        start_time = time.time()
        try:
            from vecnadb.infrastructure.llm.config import get_llm_config

            config = get_llm_config()

            from vecnadb.infrastructure.llm.utils import test_llm_connection
    ...
    async def check_embedding_service(self) -> ComponentHealth:
        ...
        start_time = time.time()
        try:
            from vecnadb.infrastructure.llm.utils import test_embedding_connection
