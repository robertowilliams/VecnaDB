...
from vecnadb.modules.data.methods import get_dataset_data
...
async def main():
    ...
    await vecnadb.add([explanation_file_path_nlp], dataset_name="NLP")
    ...
    await vecnadb.add([explanation_file_path_quantum], dataset_name="QUANTUM", user=test_user)

    nlp_cognify_result = await vecnadb.cognify(["NLP"], user=default_user)
    quantum_cognify_result = await vecnadb.cognify(["QUANTUM"], user=test_user)
    ...
    cognify_error = False
    try:
        await vecnadb.cognify(datasets=[test_user_dataset_id], user=default_user)
    except PermissionDeniedError:
        cognify_error = True
    ...
    await vecnadb.delete(data_id=text_data_id, dataset_id=test_user_dataset_id, user=test_user)
