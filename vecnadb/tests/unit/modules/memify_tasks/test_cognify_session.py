...
from vecnadb.tasks.memify.cognify_session import cognify_session
from vecnadb.exceptions import CogneeValidationError, CogneeSystemError
...
async def test_cognify_session_success():
    ...
    with (
        patch("vecnadb.add", new_callable=AsyncMock) as mock_add,
        patch("vecnadb.cognify", new_callable=AsyncMock) as mock_cognify,
    ):
...
async def test_cognify_session_add_failure():
    """Test cognification handles vecnadb.add failure."""
    session_data = "Session ID: test\n\nQuestion: test?"

    with (
        patch("vecnadb.add", new_callable=AsyncMock) as mock_add,
        patch("vecnadb.cognify", new_callable=AsyncMock),
    ):
...
async def test_cognify_session_cognify_failure():
    ...
    with (
        patch("vecnadb.add", new_callable=AsyncMock),
        patch("vecnadb.cognify", new_callable=AsyncMock) as mock_cognify,
    ):
