...
from vecnadb.modules.users.tenants.methods import select_tenant
...
from vecnadb.modules.users.methods import create_user
from vecnadb.modules.users.permissions.methods import authorized_give_permission_on_datasets
from vecnadb.modules.users.roles.methods import add_user_to_role
from vecnadb.modules.users.roles.methods import create_role
from vecnadb.modules.users.tenants.methods import create_tenant
from vecnadb.modules.users.tenants.methods import add_user_to_tenant
from vecnadb.modules.engine.operations.setup import setup
from vecnadb.shared.logging_utils import setup_logging, CRITICAL
...
async def main():
    ...
    await vecnadb.add([text], dataset_name="AI", user=user_1)
    ...
    ai_cognify_result = await vecnadb.cognify(["AI"], user=user_1)
    ...
    await vecnadb.add([text], dataset_name="AI_COGNEE_LAB", user=user_1)
    ai_cognee_lab_cognify_result = await vecnadb.cognify(["AI_COGNEE_LAB"], user=user_1)
    ...
    user_1 = await get_user(user_1.id)
    await vecnadb.add([text], dataset_name="AI_COGNEE_LAB", user=user_1)
    await vecnadb.cognify(["AI_COGNEE_LAB"], user=user_1)
