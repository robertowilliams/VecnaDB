...
from vecnadb.infrastructure.databases.hybrid.neptune_analytics.NeptuneAnalyticsAdapter import (
...
async def main():
    ...
    vecnadb.config.set_vector_db_provider("neptune_analytics")
    vecnadb.config.set_vector_db_url(f"neptune-graph://{graph_id}")
...
async def vector_backend_api_test():
    ...
    # When URL is absent
    vecnadb.config.set_vector_db_url(None)
    with pytest.raises(OSError):
        get_vector_engine()

    # Assert invalid graph ID.
    vecnadb.config.set_vector_db_url("invalid_url")
