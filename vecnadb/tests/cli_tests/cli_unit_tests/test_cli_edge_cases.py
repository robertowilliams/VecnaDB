...
from vecnadb.cli.commands.add_command import AddCommand
from vecnadb.cli.commands.search_command import SearchCommand
from vecnadb.cli.commands.cognify_command import CognifyCommand
from vecnadb.cli.commands.delete_command import DeleteCommand
from vecnadb.cli.commands.config_command import ConfigCommand
from vecnadb.cli.exceptions import CliCommandException, CliCommandInnerException
from vecnadb.modules.data.methods.get_deletion_counts import DeletionCountsPreview
...
class TestAddCommandEdgeCases:
    ...
    @patch("vecnadb.cli.commands.add_command.asyncio.run", side_effect=_mock_run)
    ...
    @patch("vecnadb.cli.commands.add_command.asyncio.run")
...
class TestSearchCommandEdgeCases:
    ...
    @patch("vecnadb.cli.commands.search_command.asyncio.run", side_effect=_mock_run)
    ...
    def test_search_invalid_search_type_enum(self, mock_import):
        ...
        def mock_import_func(name, fromlist=None, *args, **kwargs):
            if name == "vecnadb.modules.search.types":
                module = MagicMock()
...
class TestCognifyCommandEdgeCases:
    ...
    @patch("vecnadb.cli.commands.cognify_command.asyncio.run", side_effect=_mock_run)
    ...
    @patch("vecnadb.cli.commands.cognify_command.asyncio.run")
    def test_cognify_langchain_chunker_import_error(self, mock_asyncio_run):
        ...
        def mock_import_func(name, fromlist=None, *args, **kwargs):
            if name == "cognee":
                ...
                name == "vecnadb.modules.chunking.LangchainChunker"
                and fromlist
                and "LangchainChunker" in fromlist
            ):
                raise ImportError("LangchainChunker not available")
            elif (
                name == "vecnadb.modules.chunking.TextChunker"
                and fromlist
...
class TestDeleteCommandEdgeCases:
    ...
    @patch("vecnadb.cli.commands.delete_command.get_deletion_counts")
    @patch("vecnadb.cli.commands.delete_command.fmt.confirm")
    @patch("vecnadb.cli.commands.delete_command.asyncio.run", side_effect=_mock_run)
    def test_delete_all_with_user_id(
        ...
        mock_confirm.assert_called_once_with("Delete ALL data from vecnadb?")
    ...
    @patch("vecnadb.cli.commands.delete_command.asyncio.run")
...
class TestConfigCommandEdgeCases:
    ...
    @patch("vecnadb.cli.commands.config_command.fmt.confirm")
