...
from vecnadb.low_level import DataPoint
...
async def main():
    ...
    await vecnadb.add(text)

    await vecnadb.cognify(graph_model=ProgrammingLanguage)
    ...
    await vecnadb.visualize_graph(graph_file_path)

    # Completion query that uses graph data to form context.
    completion = await vecnadb.search(SearchType.GRAPH_COMPLETION, "What is python?")
    ...
    completion = await vecnadb.search(SearchType.RAG_COMPLETION, "What is Python?")
    ...
    summaries = await vecnadb.search(SearchType.SUMMARIES, "Python")
    assert len(summaries) != 0, "Summaries search didn't return any results."
    print("Summary results are:")
    for summary in summaries:
        print(summary)

    chunks = await vecnadb.search(SearchType.CHUNKS, query_text="Python")
