...
async def main():
    ...
    await vecnadb.add(text_1, dataset_name)
    ...
    completion_sum = await vecnadb.search(
        query_type=SearchType.GRAPH_SUMMARY_COMPLETION,
        query_text="Next to which country is Germany located?",
        save_interaction=True,
    )
    completion_triplet = await vecnadb.search(
    ...
    for name, search_results in [
        ...
        from vecnadb.context_global_variables import backend_access_control_enabled
    ...
    await vecnadb.add([text], dataset_name)
