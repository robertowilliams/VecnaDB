import asyncio
import vecnadb
from vecnadb.tasks.web_scraper.config import DefaultCrawlerConfig
from vecnadb.tasks.web_scraper import cron_web_scraper_task


async def test_web_scraping_using_bs4():
    await vecnadb.prune.prune_data()
    await vecnadb.prune.prune_system()
    ...
    results = await vecnadb.search(
        ...
        query_type=vecnadb.SearchType.GRAPH_COMPLETION,
    )
...
async def test_cron_web_scraper():
    ...
    results_books = await vecnadb.search(
        "What is the price of 'A Light in the Attic' book?",
        query_type=vecnadb.SearchType.GRAPH_COMPLETION,
    )
