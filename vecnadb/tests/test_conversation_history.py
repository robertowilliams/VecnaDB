...
from vecnadb.infrastructure.databases.cache import get_cache_engine
...
async def main():
    ...
    await vecnadb.add(data=text_1, dataset_name=dataset_name)
    await vecnadb.add(data=text_2, dataset_name=dataset_name)

    await vecnadb.cognify(datasets=[dataset_name])

    user = await get_default_user()

    from vecnadb.memify_pipelines.create_triplet_embeddings import create_triplet_embeddings
    ...
    result_rag = await vecnadb.search(
    ...
    result_cot = await vecnadb.search(
    ...
    result_ext = await vecnadb.search(
    ...
    result_summary = await vecnadb.search(
    ...
    result_temporal = await vecnadb.search(
    ...
    result_triplet = await vecnadb.search(
    ...
    from vecnadb.modules.retrieval.utils.session_cache import (
    ...
    from vecnadb.memify_pipelines.persist_sessions_in_knowledge_graph import (
    ...
    from vecnadb.infrastructure.databases.vector.get_vector_engine import get_vector_engine
