...
from vecnadb.modules.users.exceptions import PermissionDeniedError
from vecnadb.shared.logging_utils import get_logger
from vecnadb.modules.users.methods import get_default_user, create_user
from vecnadb.modules.users.permissions.methods import authorized_give_permission_on_datasets
from vecnadb.modules.data.methods import get_dataset_data, get_datasets_by_name
from vecnadb.api.v1.exceptions import DocumentNotFoundError, DatasetNotFoundError
...
async def main():
    ...
    await vecnadb.add([text_1], dataset_name="tech_companies_1", user=default_user)
    ...
    await vecnadb.add([text_2], dataset_name="tech_companies_2", user=test_user)
    ...
    await vecnadb.add([text_3], dataset_name="tech_companies_3", user=isolation_user)

    tst = await vecnadb.cognify(["tech_companies_1"], user=default_user)
    tst2 = await vecnadb.cognify(["tech_companies_2"], user=test_user)
    tst3 = await vecnadb.cognify(["tech_companies_3"], user=isolation_user)
    print("tst", tst)
    print("tst2", tst2)
    print("tst3", tst3)
    ...
    try:
        result = await vecnadb.delete(data_id=data_to_delete_id, dataset_id=default_dataset.id)
    ...
    await vecnadb.add([text_1], dataset_name="another_dataset", user=default_user)
    await vecnadb.cognify(["another_dataset"], user=default_user)
    ...
    try:
        ...
        isolation_search_results = await vecnadb.search(
