...
from vecnadb.cli.reference import SupportsCliCommand
from vecnadb.cli import DEFAULT_DOCS_URL
import vecnadb.cli.echo as fmt
from vecnadb.cli.exceptions import CliCommandException, CliCommandInnerException
from vecnadb.modules.data.methods.get_deletion_counts import get_deletion_counts


class DeleteCommand(SupportsCliCommand):
    command_string = "delete"
    help_string = "Delete data from vecnadb knowledge base"
...
Be careful with deletion operations as they are irreversible.
    ...
    def execute(self, args: argparse.Namespace) -> None:
        try:
            ...
            if args.all:
                confirm_msg = "Delete ALL data from vecnadb?"
                operation = "all data"
            ...
            async def run_delete():
                try:
                    # NOTE: The underlying vecnadb.delete() function is currently not working as expected.
                    # This is a separate bug that this preview feature helps to expose.
                    if args.all:
                        await vecnadb.delete(dataset_name=None, user_id=args.user_id)
                    else:
                        await vecnadb.delete(dataset_name=args.dataset_name, user_id=args.user_id)
