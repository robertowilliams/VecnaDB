...
from vecnadb.infrastructure.utils.run_sync import run_sync
from vecnadb.infrastructure.files.storage import get_file_storage
from vecnadb.infrastructure.databases.graph.graph_db_interface import (
    GraphDBInterface,
    record_graph_changes,
)
from vecnadb.infrastructure.engine import DataPoint
from vecnadb.modules.storage.utils import JSONEncoder
from vecnadb.modules.engine.utils.generate_timestamp_datapoint import date_to_int
from vecnadb.tasks.temporal_graph.models import Timestamp
from vecnadb.infrastructure.databases.cache.config import get_cache_config

logger = get_logger()

cache_config = get_cache_config()
if cache_config.shared_kuzu_lock:
    from vecnadb.infrastructure.databases.cache.get_cache_engine import get_cache_engine
...
class KuzuAdapter(GraphDBInterface):
    ...
    async def push_to_s3(self) -> None:
        if os.getenv("STORAGE_BACKEND", "").lower() == "s3" and hasattr(self, "temp_graph_file"):
            from vecnadb.infrastructure.files.storage.S3FileStorage import S3FileStorage
